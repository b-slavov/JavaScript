...
## Arrays
  ...
  - [4.5](#arrays--callback-return) Use return statements in array method callbacks. It’s ok to omit the return if the function body consists of a single statement returning an expression without side effects, following [8.2](#arrows--implicit-return).
...
## Functions
  ...
  - [7.6](#es6-rest) Never use `arguments`, opt to use rest syntax `...` instead.
  ...
  - [7.12](#functions--mutate-params) Never mutate parameters.
  ...
  - [7.14](#functions--spread-vs-apply) Prefer the use of the spread operator `...` to call variadic functions.
  ...
  <a name="functions--single-responsibility"></a><a name="7.16"></a>
  - [7.16](#functions--single-responsibilityn) Functions should have a single responsibility.
  ...
  <a name="functions--internal-private-functions"></a><a name="7.17"></a>
  - [7.17](#functions--internal-private-functions) Internal private functions should be exposed to enable testing.
    ...
    >   1. End-to-end tests proving core functionality
    >   2. Proving the module won't crash given bad inputs
    >   3. Common edge cases within the module
  ...
  <a name="functions--promises-or-callbacks"></a><a name="7.18"></a>
  - [7.18](#functions--promises-or-callbacks) Promises or Callbacks?
  ...
  <a name="functions--async-with-callbacks"></a><a name="7.19"></a>
  - [7.19](#functions--async-with-callbacks) Handling heavily sequential async code with callbacks.
  ...
  <a name="functions--async-with-promises"></a><a name="7.20"></a>
  - [7.20](#functions--async-with-promises) Handling heavily sequential async code with promises.
  ...
  <a name="functions--return-data-or-callback-with-data"></a><a name="7.21"></a>
  - [7.21](#functions--return-data-or-callback-with-data) Functions must either return data OR callback with data.
  ...
  <a name="functions--return-explicit-data"></a><a name="7.22"></a>
  - [7.22](#functions--return-explicit-data) Returned data from a function must be explicit.
    ...
    ```javascript
    // bad
    // What data flows back to the callee?
  ...
  <a name="functions--connection-handling"></a><a name="7.23"></a>
  - [7.23](#functions--connection-handling) Connection Handling.
	...
	var doStuff = function(callback) {
    	...
    	connection.find('something', function(err, data) {
      	...
      	// Connection is left hanging forever!
  ...
  <a name="functions--error-handling"></a><a name="7.24"></a>
  - [7.24](#functions--error-handling) Error Handling.
  ...
  <a name="functions--no-native-errors"></a><a name="7.25"></a>
  - [7.25](#functions--no-native-errors) Don't use native errors.
	...
	> Errors don't stringify into log files:
    ...
    > Our synchronous functions return truthy values or nulll, never errors:
	...
	if (!result) return callback('Could not add numbers');
	```
  ...
  <a name="functions--careful-with-throw"></a><a name="7.26"></a>
  - [7.26](#functions--careful-with-throw) Be extremely careful when using `throw`s.

    > Our main use case for throwing errors is via public exported functions to ensure callbacks are provided when expected by other invoking modules:
	...
	> When an unhandled error occurs, the state of our application is unknown. The best course of action is to stop accepting new requests, finish serving open requests, and shutdown. Our cluster will restart the worker after it's shutdown.
  ...
  <a name="functions--avoid-try-catch"></a><a name="7.27"></a>
  - [7.27](#functions--avoid-try-catch) Avoid `try catch`.
...
## Arrow Functions
  ...
  - [8.4](#arrows--one-arg-parens) If your function takes a single argument and doesn’t use braces, omit the parentheses. Otherwise, always include parentheses around arguments for clarity and consistency. Note: it is also acceptable to always use parentheses.
...
## Variables
  ...
  - [13.1](#variables--const) Always use `const` or `let` to declare variables. Not doing so will result in global variables. We want to avoid polluting the global namespace.
...
<a name="commas--leading-trailing"></a><a name="19.1"></a>
  ...
  - [20.2](#commas--dangling) Additional trailing comma: **Yes.**
...
## jQuery
  ...
  <a name="jquery--use-simple-selectors"></a><a name="25.2"></a>
  - [26.2](#jquery--use-simple-selectors) Use Simple Selectors.

    > When it comes to simple selectors, using an ID e.g. `$('#my-id')` is the simplest and best way to select a unique element as it will defer to `document.getElementByID`. Using a tag e.g. `$('div')` will defer to `getElementsByTagName` but this is ambiguous and should only be used when selecting children or descendants of another object. Using a class e.g. `$('.my-class')` is the least favoured option of the three. Though it will defer to `getElementsByClassName` this is not supported in all older browsers.
  ...
  <a name="jquery--checking-the-dom"></a><a name="25.3"></a>
  - [26.3](#jquery--checking-the-dom) Checking the DOM has loaded.
  ...
  <a name="jquery--navigating-the-dom"></a><a name="25.4"></a>
  - [26.4](#jquery--navigating-the-dom) Navigating The DOM.

    > When traversing the DOM, where possible it is best to start with an ID and work your way up/down using `.children()`, `.find()`, `.parent()` etc. This will improve performance when navigating through dependant, especially in older browsers that don't natively support getElementsByClassName. It will also help to ensure the use of simple selectors and works best when the parent object is stored in a variable.
  ...
  <a name="jquery--cache"></a><a name="25.5"></a>
  - [26.5](#jquery--cache) Cache jQuery objects when repeating.
  ...
  <a name="jquery--chain"></a><a name="25.6"></a>
  - [25.6](#jquery--chain) Chain When Possible.
  ...
  <a name="jquery--on-and-off-events"></a><a name="25.7"></a>
  - [25.7](#jquery--on-and-off-events) Use `.on()` And `.off()` events.

    > The older jQuery methods `bind`, `live` and `delegate` along with the individual event methods, such as `click`, `submit` etc, should not be used to bind events. Instead use the `on` and `off` methods.
  ...
  <a name="jquery--multiple-events"></a><a name="25.8"></a>
  - [25.8](#jquery--multiple-events) Multiple Events.
  ...
  <a name="jquery--delegate"></a><a name="25.9"></a>
  - [25.9](#jquery--delegate) Delegate Where Possible.
  ...
  <a name="jquery--namespace"></a><a name="25.10"></a>
  - [25.10](#jquery--namespace) Namespace For More Complex Functions.
  ...
  <a name="jquery--queries"></a><a name="25.11"></a>
  - [26.11](#jquery--queries) For DOM queries use Cascading `$('.sidebar ul')` or parent > child `$('.sidebar > ul')`. [jsPerf](http://jsperf.com/jquery-find-vs-context-sel/16)

  <a name="jquery--find"></a><a name="25.12"></a>
  - [26.12](#jquery--find) Use `find` with scoped jQuery object queries.
...
## Testing
  ...
  <a name="testing--datetime-strings"></a><a name="28.3"></a>
  - [29.3](#testing--datetime-strings) Never use static datetime strings
